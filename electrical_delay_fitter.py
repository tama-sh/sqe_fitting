...
from .signal_util import group_delay, smoothing
from .circle_fitter import algebric_circle_fit

def correct_electrical_delay(omega: np.ndarray, cplx: np.ndarray, electrical_delay=None, correct_phase_offset=True):
    ...
    Args:
        ...
        electrical_delay (bool): the value of electrical_delay. if None, automatically estimate the electrical delay. default is None
        correct_phase_offset (bool): if True, automatically correct phase offset as well. default is True
    
    Returns:
        np.ndarray: complex data with electrical delay corrected
    """
    if electrical_delay is None:
        electrical_delay = estimate_electrical_delay(omega, cplx)
...
def estimate_electrical_delay(omega: np.ndarray, cplx: np.ndarray):
    """Estimate electridal delay
