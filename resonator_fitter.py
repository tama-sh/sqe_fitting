...
class resonator_fitter(object):
    ...
    def guess(self, cplx, omega, fix_electrical_delay=True, **kwargs):
        pars = self.model.guess(cplx, omega=omega, fix_electrical_delay=fix_electrical_delay, **kwargs)
        return pars
    
    def fit(self, cplx, omega, params=None, fix_electrical_delay=True, **kwargs):
        if params is None:
            params = self.guess(cplx, omega, fix_electrical_delay=fix_electrical_delay)
